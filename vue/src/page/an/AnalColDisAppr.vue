...
import values from './values/analColDisAppr';
...
export default {
  ...
  methods: {
    async getAnalColDisAppr() {
      const { $grid, forms } = this.list;
      const parameter = FormUtil.getData(forms);

      const data = await $grid
        ._useLoader(() => this.$axios.get('/an/analColDisAppr', parameter))
        .then(({ data }) => data);
    ...
    approve() {
      ...
      if (checkedRows.length > 0) {
        this.$confirm(this.$message.confirm.approved).then(() => {
          this.$eSign(() => this.$axios.put('/an/analColDisAppr/approve', parameter))
            .then(() => {
    ...
    reject() {
      ...
      if (checkedRows.length > 0) {
        this.$confirm(this.$message.confirm.rejected).then(() => {
          this.$eSignWithReason(() => this.$axios.put('/an/analColDisAppr/reject', parameter))
            .then(() => {
