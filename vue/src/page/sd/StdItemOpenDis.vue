...
import values from './values/stdItemOpenDis';
  ...
  methods: {
    async getStdOpenDis() {
      ...
      const data = await $grid
        ._useLoader(() => this.$axios.get('/sd/stdItemOpenDis', parameter))
    ...
    open() {
      ...
      if (checkedRows.length > 0) {
        this.$confirm(this.$message.confirm.opened).then(() => {
          this.$eSign(() => this.$axios.put('/sd/stdItemOpenDis/open', parameter))
            .then(() => {
    ...
    approveModalReturnDataEvent(approveInfo) {
      ...
      if (checkedRows.length > 0) {
        this.$eSign(() => this.$axios.put('/sd/stdItemOpenDis/requestDisposal', parameter))
          .then(() => {
