...
import values from './values/stdItemDisAppr';
  ...
  methods: {
    async getStdDisAppr() {
      ...
      const data = await $grid
        ._useLoader(() => this.$axios.get('/sd/stdItemDisAppr', parameter))
        .then(({ data }) => data);
    ...
    approve() {
      ...
      if (checkedRows.length > 0) {
        this.$confirm(this.$message.confirm.approved).then(() => {
          this.$eSignWithReason(() => this.$axios.put('/sd/stdItemDisAppr/approve', parameter))
            .then(() => {
    ...
    reject() {
      ...
      if (checkedRows.length > 0) {
        this.$confirm(this.$message.confirm.rejected).then(() => {
          this.$eSignWithReason(() => this.$axios.put('/sd/stdItemDisAppr/reject', parameter))
            .then(() => {
