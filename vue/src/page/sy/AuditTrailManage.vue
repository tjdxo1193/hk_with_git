...
export default {
  ...
  methods: {
    ...
    async fetchMenus() {
      ...
      const {
        data: { menus },
      } = await this.$axios.get('/dev/audit/menus', param);
      this.menu.$grid.setGridData(menus);
    },
    onClickOnMenu({ name }) {
      if (name === 'search') {
    ...
    async fetchTablesAsDropDownList() {
      const {
        data: { tables },
      } = await this.$axios.get('/dev/audit/tables');
      ...
      const dropDownList = tables.map((item) => ({
        value: item.tableNm,
        label: `${item.tableNm} (${item.tableComment})`,
      }));
    ...
    async fetchColumnsByTableNm(tableNm) {
      ...
      const {
        data: { columns },
      } = await this.$axios.get(`/dev/audit/tables/${tableNm}/columns`);
      this.column.state.currentColumns = columns;

      const dropDownColumn = GridUtil.findColumn(this.column.columns, 'columnNm');
      const dropDownList = columns.map((item) => ({
        value: item.columnNm,
