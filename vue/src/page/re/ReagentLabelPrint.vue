...
import values from './values/reagentLabelPrint';
...
export default {
  ...
  methods: {
    async getReagentLabelPrint() {
      const { $grid, forms } = this.list;
      const parameter = FormUtil.getData(forms);
      const data = await $grid
        ._useLoader(() => this.$axios.get('/re/reagentLabelPrint', parameter))
        .then(({ data }) => data);
    ...
    onClickButton({ name }) {
      if (name === 'search') {
        return this.getReagentLabelPrint();
      }
      if (name === 'fileSave') {
    ...
    print() {
      ...
      this.$confirm(this.$message.confirm.printed).then(() => {
        if (selectedItem.length > 0) {
          ...
          this.$eSign(() => this.$axios.put('/re/reagentLabelPrint', selectedItem[0].item))
            .then(() => {
              this.getReagentLabelPrint().then(() => {
