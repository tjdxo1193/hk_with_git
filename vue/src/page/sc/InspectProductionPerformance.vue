  ...
  methods: {
    ...
    async fetchDetailGrid(params) {
      ...
      const data = await $grid
        ._useLoader(() =>
          this.$axios.get(
            `/sc/inspectProductionPerformance/${params.pitmCd}/${params.pdtOrderNo}/${params.batchNo}/${params.ispReqDt}`,
          ),
        )
        .then(({ data }) => data)
        .catch(() => {
    ...
    isReqDtLessThanYesterday(checkedRows) {
      ...
      return checkedRows.filter((row) => row.ispReqDt > yesterday).length > 0 ? true : false;
    },
    sendIF(checkedRows) {
    ...
    makeParam(event) {
      const pitmCd = event.item.pitmCd;
      const pdtOrderNo = event.item.pdtOrderNo;
      const batchNo = event.item.batchNo;
      const ispReqDt = event.item.ispReqDt;
      return { pitmCd, pdtOrderNo, batchNo, ispReqDt };
    },
    record(item) {
