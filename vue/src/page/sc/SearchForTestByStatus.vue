...
import values from './values/searchForTestByStatus.js';
  ...
  methods: {
    async fetchSearchGridWithForm() {
      ...
      const data = await $grid
        ._useLoader(() => this.$axios.get('/sc/searchForTestByStatus', param))
        .then(({ data }) => data)
        .catch(() => {
    ...
    async fetchAitmSpecGrid(ansIdx) {
      ...
      const data = await $grid
        ._useLoader(() => this.$axios.get('/sc/searchForTestByStatus/spec', { ansIdx }))
        .then(({ data }) => data)
        .catch(() => {
          this.$error(this.$message.error.fetchData);
        });
