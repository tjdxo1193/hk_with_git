...
import values from './values/glassOpenDis';
...
export default {
  ...
  methods: {
    async getGlassOpenDis() {
      const { $grid, forms } = this.list;
      const parameter = FormUtil.getData(forms);
      const data = await $grid
        ._useLoader(() => this.$axios.get('/gl/glassOpenDis', parameter))
    ...
    open() {
      ...
      if (checkedRows.length > 0) {
        this.$confirm(this.$message.confirm.opened).then(() => {
          this.$eSign(() => this.$axios.put('/gl/glassOpenDis/open', parameter))
            .then(() => {
    ...
    disposal() {
      ...
      if (checkedRows.length > 0) {
        this.$confirm(this.$message.confirm.disposal).then(() => {
          this.$eSign(() => this.$axios.put('/gl/glassOpenDis/disposal', parameter))
            .then(() => {
              this.$info(this.$message.info.disposal);
              this.getGlassOpenDis();
            })
            .catch(() => {
              this.$error(this.$message.error.updateData);
    ...
    showAndHideDpsReaColumn(mngProcCd) {
      ...
      if (mngProcCd === openState) {
        return this.list.$grid.hideColumnByDataField(['dpsDt', 'dpsRea']);
      }
      if (mngProcCd === disposalRequestState) {
        return this.list.$grid.showColumnByDataField(['dpsDt', 'dpsRea']);
      }
    },
  ...
  computed: {
    computedListColumns() {
      ...
      const disposalReaColumns = ['dpsDt', 'dpsRea'];
