...
import values from './values/glassReceipt';
  ...
  methods: {
    async getGlassItem() {
      ...
      const data = await this.list.$grid
        ._useLoader(() => this.$axios.get('/gl/glassReceipt', parameter))
        .then(({ data }) => data);

      this.list.$grid.setGridData(data);
    },
    getGlassItemByRitmEtrIdx(event) {
    ...
    create() {
      ...
      this.$confirm(this.$message.confirm.saveData).then(() => {
        this.$eSign(() => this.$axios.post('/gl/glassReceipt', param))
          .then(({ data }) => {
    ...
    update() {
      ...
      this.$confirm(this.$message.confirm.updated).then(() => {
        this.$eSignWithReason(() => this.$axios.put('/gl/glassReceipt', param))
          .then(() => {
    ...
    delete() {
      ...
      this.$confirm(this.$message.confirm.deleted).then(() => {
        this.$eSignWithReason(() => this.$axios.put('/gl/glassReceipt/delete', param))
          .then(() => {
    ...
    setEnableForms() {
      ...
      this.setDisabledColumn(selecteItem[0].item.expirDtChk);
      this.getGlassItemByRitmEtrIdx(selecteItem[0]);
    },
    enableButtons() {
