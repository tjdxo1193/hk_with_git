...
import values from './values/monitorSpecAppr';
  ...
  methods: {
    async fetchMonitorSpecVersion() {
      ...
      const data = await $grid
        ._useLoader(() => this.$axios.get('ms/monitorSpecAppr/version', parameter))
        .then(({ data }) => data);
    ...
    async fetchMItemSpecAItemList({ item }) {
      ...
      const data = await $grid
        ._useLoader(() => this.$axios.get('ms/monitorSpecAppr/aItem', item))
        .then(({ data }) => data);
      $grid.setGridData(data);
    },
    ...
    approve() {
      const checkedRows = this.versionList.$grid.getCheckedRowItemsAll();
      this.$eSign(() => this.$axios.put('/ms/monitorSpecAppr/approval', checkedRows))
        .then(() => {
    ...
    reject({ rjtRea }) {
      ...
      this.$eSign(() => this.$axios.put('/ms/monitorSpecAppr/reject', parameter))
        .then(() => {
