...
export default {
  ...
  methods: {
    ...
    async approveUseModalReturnDataEvent(data) {
      const { forms } = this.inputForm;
      const param = FormUtil.getData(forms);
      await this.$axios.put('/st/stabSampleUsage', param);
      await this.$eSign(() =>
    ...
    setButtonsEnable(status) {
      ...
      if (status === TEMP_SAVE) {
        ...
        FormUtil.enable(forms, ['useSmpVol', 'useDt', 'strgPla', 'usePps', 'sampleSearch']);
      }
      if (
        status === REQUEST_USE ||
        ...
        FormUtil.disable(forms, ['sampleSearch']);
