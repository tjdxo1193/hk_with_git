...
export default {
  ...
  methods: {
    ...
    stopRequest({ rjtReaDiv, rjtRea }) {
      ...
      this.getSbtAnsForValidate().then(() =>
    ...
    async getSbtAnsForValidate() {
      const { forms } = this.stabInfoRegForm;
      const parameter = FormUtil.getData(forms);

      const data = await this.$axios
        .get('st/stabPlan/getSbtAnsForValidate', parameter)
        .then(({ data }) => data);

      if (!data) {
        this.$warn(this.$message.warn.noStabPlanDetailData);
        throw new Error(this.$message.warn.noStabPlanDetailData);
      }
    },
    save() {
      // 저장
