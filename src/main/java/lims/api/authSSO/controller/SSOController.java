...
public class SSOController {
    ...
    public ResponseEntity<Void> toSSOLoginPage() {
        HttpHeaders headers = new HttpHeaders();
        headers.add("location", ssoProperty.getSSOLoginPageUrl());
    ...
    public ResponseEntity<Void> SSOAuthenticate(SSOLoginRequest request, HttpSession session, RedirectAttributes redirectAttr) {
        ...
        try {
            ...
            SafeAccountAuthentication safeAccountAuthentication = SafeAccountAuthentication.builder()
                    ...
                    .sso(UseType.Y)
                    .build();
            TokenAuthentication tokenAuthentication = authenticator.authenticate(safeAccountAuthentication);
    ...
    private void assertExistsPlantCode(String plantCode) {
        if (StringUtils.isEmpty(plantCode)) {
            throw new RuntimeException("No exists plant code");
        }
    }

    private void assertExistsLoginId(String loginId) {
        if (StringUtils.isEmpty(loginId)) {
            throw new RuntimeException("No exists login id");
        }
    }
