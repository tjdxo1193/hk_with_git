...
public class CommonCodeManageController {
    ...
    public ResponseEntity<List<CommonCodeVO>> getUpperCode(@AuthToken Token token, CommonCodeVO request){
    ...
    public ResponseEntity<List<CommonDetailCodeVO>> getDetailCode(@AuthToken Token token, CommonDetailCodeVO request){
        String jwt = token.getJwt();
        request.setPlntCd(jwtResolver.getPlantCode(jwt));
